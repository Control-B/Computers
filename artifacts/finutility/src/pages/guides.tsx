...
const CATEGORY_COLORS: Record<string, string> = {
  "Laptops & PCs": "bg-blue-100 text-blue-700",
  "Components": "bg-purple-100 text-purple-700",
  "Networking": "bg-cyan-100 text-cyan-700",
  "Smartphones": "bg-indigo-100 text-indigo-700",
        ...
        <section className="relative overflow-hidden bg-slate-950 text-white pt-20 pb-24 border-b border-border">
          ...
          <div className="container mx-auto px-4 md:px-8 relative z-10 text-center">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-blue-500/20 border border-blue-500/30 text-blue-300 text-sm font-medium mb-6">
              ...
              Computer & Electronics Expert Guides
            </div>
            <h1 className="text-4xl md:text-5xl font-black mb-4">Problem → Solution → Benefit</h1>
            <p className="text-lg text-slate-300 max-w-xl mx-auto mb-8">
              Every guide solves a real tech problem, provides a practical solution, shows the benefit, and proves it with a real-world STAR scenario. No fluff — just answers.
            </p>
            <div className="relative max-w-lg mx-auto">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-white/50" />
              <Input
                type="text"
                placeholder="Search guides by topic, device, or problem..."
                className="h-12 pl-12 text-base rounded-full bg-white/10 border-white/20 text-white placeholder:text-white/40 focus-visible:ring-blue-500"
                value={search}
        ...
        <div className="container mx-auto px-4 md:px-8 py-12 max-w-6xl">
          ...
          <div className="flex flex-wrap gap-2 my-8">
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-semibold transition-colors ${activeCategory === null ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-700 hover:bg-slate-200"}`}
            >
            ...
            {categories.map((cat) => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-semibold transition-colors ${activeCategory === cat ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-700 hover:bg-slate-200"}`}
              >
                {cat}
              </button>
            ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 my-8">
            {filtered.map((guide) => (
              <Link key={guide.slug} href={`/${guide.slug}`}>
                  <div className="group h-full flex flex-col bg-white rounded-2xl border border-border hover:border-blue-300 hover:shadow-lg transition-all cursor-pointer overflow-hidden">
                  <div className="p-6 flex-1 flex flex-col">
                    ...
                    <h3 className="font-black text-slate-900 mb-2 group-hover:text-blue-700 transition-colors leading-snug flex-1">
                      {guide.title}
                    </h3>
                  ...
                  {guide.calculatorHref && (
                    <div className="px-6 py-3 bg-blue-50 border-t border-blue-100 flex items-center gap-2">
                      <span className="text-xs text-blue-700 font-medium">Related tool:</span>
                      <span className="text-xs font-bold text-blue-800">{guide.calculatorLabel}</span>
                    </div>
                  )}
                </div>
              </Link>
